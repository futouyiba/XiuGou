...
/// When to call: This function can be called after calling [createEngine].
/// Caution: This function is valid only when the [muteAllPlayStreamVideo] function is set to `NO`.
...
///
/// @param mute Whether it is possible to receive the video data of the specified remote user when streaming, "YES" means prohibition, "NO" means receiving, the default value is "NO".
/// @param streamID Stream ID.
...
///
/// @param enable Whether to turn on frame order detection, YES: enable check poc,not support B frames, NO: disable check poc, support B frames.
